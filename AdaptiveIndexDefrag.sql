...
'AND i.object_id = ' + CAST(@objectID AS NVARCHAR(10)) + ' AND i.index_id = ' + CAST(@indexID AS NVARCHAR(10)) + ';'
			...
			BEGIN			
				...
				IF @dataCompression IS NULL  and @currCompression is not null
					AND (@dealMaxPartition IS NULL OR (@dealMaxPartition IS NOT NULL AND @partitionCount = 1))
					AND @ixtype IN (1,2)
					AND (@sqlmajorver > 13 OR (@sqlmajorver = 13 AND @sqlbuild >= 4001) OR @editionCheck = 1) -- SQL 2016 SP1 or higher	for compression in non-EE
				...
				IF @dataCompression IS NULL  and @currCompression is not null AND @ixtype IN (1,2)
					AND @partitionCount > 1 AND @dealMaxPartition IS NOT NULL AND @editionCheck = 1
			...
			BEGIN
				...
				SELECT TOP 1 @statsobjectID=@objectID, @statsName = statsName, @partitionNumber = partitionNumber, @stats_norecompute = [no_recompute], @stats_isincremental = [is_incremental]
				FROM dbo.tbl_AdaptiveIndexDefrag_Stats_Working
				...
				IF @debugMode = 1
				BEGIN
					SELECT @debugMessage = '     Found a row modification counter of ' + CONVERT(NVARCHAR(11), @rowmodctr) + ' and ' + CONVERT(NVARCHAR(11), @record_count) + ' rows' + CASE WHEN ISNULL(@stats_isincremental,0) = 1 THEN ' on partition ' + CONVERT(NVARCHAR(10), @partitionNumber) ELSE '' END + '...';
					RAISERROR(@debugMessage, 0, 42) WITH NOWAIT;
				END
		...
		BEGIN
			...
			BEGIN
				...
				IF @Exec_Print = 1
				BEGIN
					/* Added in Objectid and ObjecName for logging dmoloney*/
				...
				IF @debugMode = 1
				BEGIN
					SELECT @debugMessage = '     Found a row modification counter of ' + CONVERT(NVARCHAR(11), @rowmodctr) + ' and ' + CONVERT(NVARCHAR(11), CASE WHEN @rows IS NOT NULL AND @rows < @record_count THEN @rows ELSE @record_count END) + ' rows' + CASE WHEN @stats_isincremental = 1 THEN ' on partition ' + CONVERT(NVARCHAR(10), @partitionNumber) ELSE '' END + '...';
					RAISERROR(@debugMessage, 0, 42) WITH NOWAIT;
					--select @debugMessage
				END
