...
IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_NAME = 'usp_whatsup')
	EXEC ('CREATE PROC dbo.usp_whatsup AS SELECT ''stub version, to be replaced''')
GO
/****
-- 2012-04-07 Pedro Lopes (Microsoft) (http://aka.ms/tigertoolbox/)
-- Change Log :
		Kin Shah (https://dba.stackexchange.com/users/8783/kin): 
					- changed the variable @sqlcmd NVARCHAR(500) to NVARCHAR(max) to prevent "Common table expression defined but not used." error.
					- removed the extra "," that was throwing parsing error
					- Added create proc and alter to allow new changes to the SP if the SP is already present. 
					- created variable @UpTime1. Since @uptime is already declared, you cannot use dupe variables.

****/
ALTER PROCEDURE dbo.usp_whatsup @uptime BIT = 1
	,@requests BIT = 1
	,@blocking BIT = 1
	,@spstats BIT = 0
	,@qrystats BIT = 0
...
BEGIN
	...
	EXECUTE sp_executesql @sqlcmd
		...
		,@UpTimeOUT = @UpTime1 OUTPUT;

	SELECT 'Uptime_Information' AS [Information]
		,GETDATE() AS [Current_Time]
		,@StartDate AS Last_Startup
		,CONVERT(VARCHAR(4), @UpTime1 / 60 / 24) + 'd ' + CONVERT(VARCHAR(4), @UpTime1 / 60 % 24) + 'h ' + CONVERT(VARCHAR(4), @UpTime1 % 60) + 'm' AS Uptime
...
BEGIN
	SELECT 'Waiter_Blocking_Report' AS [Information]
		...
		*/
		...
		,(
			SELECT REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(qt2.TEXT, NCHAR(1), N'?'), NCHAR(2), N'?'), NCHAR(3), N'?'), NCHAR(4), N'?'), NCHAR(5), N'?'), NCHAR(6), N'?'), NCHAR(7), N'?'), NCHAR(8), N'?'), NCHAR(11), N'?'), NCHAR(12), N'?'), NCHAR(14), N'?'), NCHAR(15), N'?'), NCHAR(16), N'?'), NCHAR(17), N'?'), NCHAR(18), N'?'), NCHAR(19), N'?'), NCHAR(20), N'?'), NCHAR(21), N'?'), NCHAR(22), N'?'), NCHAR(23), N'?'), NCHAR(24), N'?'), NCHAR(25), N'?'), NCHAR(26), N'?'), NCHAR(27), N'?'), NCHAR(28), N'?'), NCHAR(29), N'?'), NCHAR(30), N'?'), NCHAR(31), N'?') AS [text()]
			FROM sys.dm_exec_sql_text(COALESCE(er2.sql_handle, ec2.most_recent_sql_handle)) AS qt2
			FOR XML PATH('')
				,TYPE
			) AS [blocker_batch]
		,(
			SELECT REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(SUBSTRING(qt2.TEXT, 1 + (
																																		CASE 
																																			WHEN er2.statement_start_offset = 0
																																				THEN 0
																																			ELSE er2.statement_start_offset / 2
																																			END
																																		), 1 + (
																																		CASE 
																																			WHEN er2.statement_end_offset = - 1
																																				THEN DATALENGTH(qt2.TEXT)
																																			ELSE er2.statement_end_offset / 2
																																			END - (
																																			CASE 
																																				WHEN er2.statement_start_offset = 0
																																					THEN 0
	...
	LEFT JOIN sys.dm_exec_requests(NOLOCK) er ON es.session_id = er.session_id
	LEFT JOIN sys.dm_exec_connections(NOLOCK) ec ON es.session_id = ec.session_id
	LEFT JOIN sys.dm_os_tasks(NOLOCK) ot ON er.session_id = ot.session_id
		AND er.request_id = ot.request_id
	LEFT JOIN sys.dm_exec_sessions(NOLOCK) es2 ON er.blocking_session_id = es2.session_id
	LEFT JOIN sys.dm_exec_requests(NOLOCK) er2 ON es2.session_id = er2.session_id
	LEFT JOIN sys.dm_exec_connections(NOLOCK) ec2 ON es2.session_id = ec2.session_id
	LEFT JOIN (
		...
		SELECT waiting_task_address
			...
			,CASE 
				...
				WHEN [wait_type] LIKE 'LCK%'
					...
					AND ISNUMERIC(RIGHT(LEFT([resource_description], CHARINDEX('dbid=', [resource_description], CHARINDEX('pageid=', [resource_description]) + 6) - 1), CHARINDEX('=', REVERSE(RTRIM(LEFT([resource_description], CHARINDEX('dbid=', [resource_description], CHARINDEX('pageid=', [resource_description]) + 6) - 1)))))) = 1
					THEN CAST(RIGHT(LEFT([resource_description], CHARINDEX('dbid=', [resource_description], CHARINDEX('pageid=', [resource_description]) + 6) - 1), CHARINDEX('=', REVERSE(RTRIM(LEFT([resource_description], CHARINDEX('dbid=', [resource_description], CHARINDEX('pageid=', [resource_description]) + 6) - 1))))) AS BIGINT)
				ELSE NULL
				END AS pageid
			,CASE 
				WHEN [wait_type] LIKE 'LCK%'
					AND [resource_description] LIKE '%associatedObjectId%'
					AND ISNUMERIC(RIGHT([resource_description], CHARINDEX('=', REVERSE([resource_description])) - 1)) = 1
					THEN CAST(RIGHT([resource_description], CHARINDEX('=', REVERSE([resource_description])) - 1) AS BIGINT)
				ELSE NULL
				END AS [objid]
			,ROW_NUMBER() OVER (
		...
		) owt ON ot.task_address = owt.waiting_task_address
		AND owt.row_num = 1
	...
	WHERE es.session_id <> @@SPID
		...
		AND (
			es.session_id IN (
				SELECT er3.blocking_session_id
				FROM sys.dm_exec_requests(NOLOCK) er3
				)
...
BEGIN
	SET @sqlcmd = 
		N'SELECT CASE WHEN ps.database_id = 32767 THEN ''ResourceDB'' ELSE DB_NAME(ps.database_id) END AS DatabaseName, 
...
BEGIN
	SET @sqlcmd = 
		N'SELECT CASE WHEN CONVERT(int,pa.value) = 32767 THEN ''ResourceDB'' ELSE DB_NAME(CONVERT(int,pa.value)) END AS DatabaseName,
...
BEGIN
	SET @sqlcmd = 
		N'SELECT CASE WHEN ts.database_id = 32767 THEN ''ResourceDB'' ELSE DB_NAME(ts.database_id) END AS DatabaseName, 
...
BEGIN
	SET @sqlcmd = 
		N'SELECT CASE WHEN fs.database_id = 32767 THEN ''ResourceDB'' ELSE DB_NAME(fs.database_id) END AS DatabaseName, 
